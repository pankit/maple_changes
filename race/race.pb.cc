...
const StaticRaceEventProto& StaticRaceEventProto::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_race_2frace_2eproto();  return *default_instance_;
}
...
bool RaceDBProto::IsInitialized() const {
  ...
  for (int i = 0; i < static_event_size(); i++) {
    if (!this->static_event(i).IsInitialized()) return false;
  }
  for (int i = 0; i < static_race_size(); i++) {
    if (!this->static_race(i).IsInitialized()) return false;
  }
  for (int i = 0; i < race_size(); i++) {
    if (!this->race(i).IsInitialized()) return false;
  }
  return true;
}
